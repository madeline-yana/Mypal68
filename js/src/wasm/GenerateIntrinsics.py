...
def main(c_out, yaml_path):
    ...
    for i in range(len(data)):
        ...
        contents += "    M("+op['op']+", \""+op['export']+"\", "+sa['name']+", "+sa['type']+", "+op['entry']+", "+str(i)+")\\\n"
    contents += "\n"

    for op in data:
        # Define DECLARE_INTRINSIC_SAS_PARAM_VALTYPES_<op> as:
        # `{ValType::I32, ValType::I32, ...}`.
        contents += "#define DECLARE_INTRINSIC_SAS_PARAM_VALTYPES_"+op['op']
        contents += " {ValType::"+", ValType::".join(op['params'])+"}\n"
        # Define DECLARE_INTRINSIC_PARAM_TYPES_<op> as:
        # `<num_types>, {_PTR, _I32, ..., _PTR, _END}`.
        sas_types = "{_PTR, _"+", _".join(op['params'])+", _PTR, _END}"
        num_types = len(op["params"]) + 2
        contents += "#define DECLARE_INTRINSIC_PARAM_TYPES_"+op['op']+" "+str(num_types)+", "+sas_types+"\n"
